...
public class MazeSolverRecursivoCompleto implements MazeSolver {
    ...
    public List<Cell> solve(Cell[][] maze, Cell start, Cell end) {
        bestPath.clear();
        steps = 0;
        explore(new ArrayList<>(), maze, start.getRow(), start.getCol(), end);
        return bestPath;
    }

    private void explore(List<Cell> currentPath, Cell[][] maze, int r, int c, Cell end) {
        ...
        int[] dc = {0, 0, -1, 1};
        for (int i = 0; i < 4; i++) {
            explore(currentPath, maze, r + dr[i], c + dc[i], end);
        }
